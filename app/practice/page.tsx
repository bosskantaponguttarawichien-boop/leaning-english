        ...
        <main className="flex min-h-screen flex-col items-center bg-zinc-50 p-6 md:p-24 tracking-tight">
            <div className="w-full max-w-4xl flex flex-col gap-10">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-zinc-500 font-medium">Select a way to improve your English skills.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {modes.map((mode, idx) => (
                        <Link
                            key={idx}
                            href={mode.href}
                            className={`group relative p-8 rounded-3xl border ${mode.borderColor} bg-white shadow-xl shadow-zinc-200/50 transition-all hover:scale-[1.02] active:scale-[0.98] ${mode.disabled ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
                        >
                            <div className="flex flex-col gap-4">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black text-zinc-900">{mode.title}</h3>
                                    <p className="text-zinc-500 font-medium mt-1">{mode.description}</p>
                                </div>
                            </div>
                            {!mode.disabled && (
                                <div className="absolute bottom-8 right-8 text-zinc-300 group-hover:text-zinc-900 transition-colors">
